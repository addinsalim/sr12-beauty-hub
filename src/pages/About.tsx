    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden py-14 sm:py-24">
        ...
        <div className="container relative mx-auto px-4 text-center">
          <h1 className="font-display text-3xl sm:text-4xl font-bold text-foreground md:text-5xl lg:text-6xl opacity-0 animate-blur-in">
            Tentang SR12 Store
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm sm:text-base text-muted-foreground leading-relaxed opacity-0 animate-blur-in" style={{ animationDelay: '0.15s' }}>
      ...
      <div className="container mx-auto px-4 py-10 sm:py-20">
        <h2 className="mb-8 sm:mb-14 text-center font-display text-2xl sm:text-3xl font-bold text-foreground accent-line-center">Nilai Kami</h2>
        <div className="grid grid-cols-2 gap-4 sm:gap-8 md:grid-cols-2 lg:grid-cols-4">
          {values.map((v, i) => (
            <div
              key={v.title}
              className="group glow-ring rounded-2xl glass p-5 sm:p-8 text-center shadow-card transition-all duration-500 hover:-translate-y-2 hover:shadow-glow-lg opacity-0 animate-slide-up"
              style={{ animationDelay: `${i * 0.1}s` }}
            >
              <div className="mx-auto mb-4 sm:mb-5 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-primary/10 transition-all duration-300 group-hover:bg-primary/20 group-hover:shadow-glow group-hover:scale-110">
                <v.icon className="h-6 w-6 sm:h-7 sm:w-7 text-primary" />
              </div>
              <h3 className="mb-2 sm:mb-3 font-display text-base sm:text-lg font-semibold text-card-foreground">{v.title}</h3>
              <p className="text-xs sm:text-sm leading-relaxed text-muted-foreground">{v.desc}</p>
            </div>
          ))}
        </div>
      </div>
