...
const AdminReports = () => {
  ...
  const { data: orders = [], isLoading } = useQuery({
    queryKey: ['admin-report-orders', period, year, month],
    queryFn: async () => {
      let start, end;
      if (period === 'yearly') {
        start = `${Number(year) - 4}-01-01`;
        ...
        end = `${year}-${paddedMonth}-${daysInMonth}T23:59:59`;
      }
  ...
  const { data: orderItems = [] } = useQuery({
    queryKey: ['admin-report-items', period, year, month],
    queryFn: async () => {
      let start, end;
      if (period === 'yearly') {
        start = `${Number(year) - 4}-01-01`;
  ...
  // Aggregated revenue based on period
  const aggregatedRevenue = useMemo(() => {
    if (period === 'daily') {
      const days = getDaysInMonth(Number(year), Number(month));
      const map = Array.from({ length: days }, (_, i) => ({ label: `Tgl ${i + 1}`, revenue: 0, orders: 0 }));
      orders.forEach(o => {
    ...
    if (period === 'weekly') {
      ...
      const map = Array.from({ length: weeks }, (_, i) => ({ label: `Pekan ${i + 1}`, revenue: 0, orders: 0 }));
      orders.forEach(o => {
        if (o.status === 'cancelled') return;
        const d = new Date(o.created_at).getDate();
        const w = Math.ceil(d / 7) - 1;
        if (map[w]) {
          map[w].revenue += Number(o.total);
          map[w].orders += 1;
        }
      });
      return map;
    ...
    if (period === 'monthly') {
      const map = Array.from({ length: 12 }, (_, i) => ({ label: MONTHS[i], revenue: 0, orders: 0 }));
      orders.forEach(o => {
        if (o.status === 'cancelled') return;
    ...
    if (period === 'yearly') {
      const yStr = Number(year);
      const map = Array.from({ length: 5 }, (_, i) => ({ label: String(yStr - 4 + i), revenue: 0, orders: 0 }));
      orders.forEach(o => {
        if (o.status === 'cancelled') return;
        const oYear = new Date(o.created_at).getFullYear();
        const idx = oYear - (yStr - 4);
        if (idx >= 0 && idx < 5 && map[idx]) {
          map[idx].revenue += Number(o.total);
          map[idx].orders += 1;
        }
      });
      return map;
  ...
  const getPeriodLabel = () => {
    if (period === 'daily') return `Harian (${MONTHS[Number(month)]} ${year})`;
    if (period === 'weekly') return `Pekanan (${MONTHS[Number(month)]} ${year})`;
    if (period === 'monthly') return `Bulanan (${year})`;
    if (period === 'yearly') return `Tahunan (${Number(year) - 4} - ${year})`;
    return '';
  };

  const getColLabel = () => {
    if (period === 'daily') return 'Tanggal';
    if (period === 'weekly') return 'Pekan';
    if (period === 'monthly') return 'Bulan';
    if (period === 'yearly') return 'Tahun';
    return 'Waktu';
  };

  // Export helpers
  const getReportData = () => aggregatedRevenue.map(m => ({
    [getColLabel()]: m.label,
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    const title = `Laporan Penjualan SR12 - ${getPeriodLabel()}`;
    ...
    autoTable(doc, {
      startY: 48,
      head: [[getColLabel(), 'Jumlah Pesanan', 'Pendapatan']],
      body: aggregatedRevenue.map(m => [m.label, m.orders, formatRupiah(m.revenue)]),
      theme: 'grid',
    ...
    doc.save(`Laporan-${period}-SR12-${year}.pdf`);
  };
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws1, `Penjualan ${period}`);
    XLSX.utils.book_append_sheet(wb, ws2, 'Produk Terlaris');
    XLSX.writeFile(wb, `Laporan-${period}-SR12-${year}.xlsx`);
  };
  ...
  const exportDoc = () => {
    ...
    html += `<h1>Laporan Penjualan SR12 - ${getPeriodLabel()}</h1>`;
    ...
    html += `<h2>Penjualan ${getPeriodLabel()}</h2><table><tr><th>${getColLabel()}</th><th>Pesanan</th><th>Pendapatan</th></tr>`;
    aggregatedRevenue.forEach(m => { html += `<tr><td>${m.label}</td><td>${m.orders}</td><td>${formatRupiah(m.revenue)}</td></tr>`; });
    html += `</table>`;
    if (topProducts.length) {
    ...
    a.download = `Laporan-${period}-SR12-${year}.doc`;
    a.click();
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Select value={period} onValueChange={(val: any) => setPeriod(val)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="weekly">Pekanan</SelectItem>
      ...
      <Tabs defaultValue="revenue" className="space-y-4">
        ...
        <TabsContent value="revenue">
          <Card>
            <CardHeader><CardTitle className="text-base">Pendapatan {getPeriodLabel()}</CardTitle></CardHeader>
            <CardContent>
              <ChartContainer config={chartConfig} className="h-[350px] w-full">
                <BarChart data={aggregatedRevenue} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
        ...
        <TabsContent value="orders">
          <Card>
            <CardHeader><CardTitle className="text-base">Jumlah Pesanan {getPeriodLabel()}</CardTitle></CardHeader>
            <CardContent>
              <ChartContainer config={chartConfig} className="h-[350px] w-full">
                <LineChart data={aggregatedRevenue} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                  <XAxis dataKey="label" className="text-xs" />
