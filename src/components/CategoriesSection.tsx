    ...
    <section className="py-10 sm:py-16 md:py-24">
      <div className="container mx-auto px-4">
        <div className="mb-6 sm:mb-10 text-center">
          ...
          <p className="mt-1.5 sm:mt-2 text-sm sm:text-base text-muted-foreground">{t.categories.subtitle}</p>
        </div>

        {/* Horizontal scroll on mobile, grid on desktop */}
        <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide sm:grid sm:grid-cols-3 sm:gap-6 sm:overflow-visible sm:pb-0">
          {categories.map((cat, i) => (
            <Link
              key={cat.name}
              to={cat.href}
              className="group relative min-w-[75vw] shrink-0 snap-center overflow-hidden rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-card transition-all duration-300 active:scale-[0.98] hover:-translate-y-1 hover:shadow-elegant opacity-0 animate-fade-in sm:min-w-0 sm:shrink"
            ...
            >
              ...
              <div className="relative flex flex-col items-center text-center">
                <div className="mb-3 sm:mb-4 h-28 w-28 sm:h-40 sm:w-40 overflow-hidden rounded-full bg-secondary/50 p-3 sm:p-4">
                ...
                <h3 className="mb-1 font-display text-lg sm:text-xl font-bold text-card-foreground">{cat.name}</h3>
                <p className="mb-3 sm:mb-4 text-xs sm:text-sm text-muted-foreground">{cat.description}</p>
